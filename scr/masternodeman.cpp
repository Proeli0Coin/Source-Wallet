...
std::vector<pair<int, CMasternode> > CMasternodeMan::GetMasternodeRanks(int64_t nBlockHeight, int minProtocol)
{
	...
	BOOST_FOREACH(CMasternode& mn, vMasternodes) {
		...
		if (!mn.IsEnabled()) {
			vecMasternodeScores.push_back(make_pair(4999, mn));
			continue;
		}
...
void CMasternodeMan::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv)
{
	...
	if (strCommand == "mnb") { //Masternode Broadcast
		...
		if (!mnb.CheckAndUpdate(nDoS)) {
			if (nDoS > 0)
				Misbehaving(pfrom->GetId(), nDoS, 24);
		...
		if (mnb.CheckInputsAndAdd(nDoS)) {
			...
			if (nDoS > 0)
				Misbehaving(pfrom->GetId(), nDoS, 26);
		}
	}
		...
		if (nDoS > 0) {
			...
			Misbehaving(pfrom->GetId(), nDoS, 27);
		}
		else {
	 ...
	 */
	 ...
	 // Light version for OLD MASSTERNODES - fake pings, no self-activation
		...
		if (pubkeyScript.size() != 25) {
			...
			Misbehaving(pfrom->GetId(), 100, 31);
			return;
		}
		...
		if (pubkeyScript2.size() != 25) {
			...
			Misbehaving(pfrom->GetId(), 100, 32);
			return;
		}

		if (!vin.scriptSig.empty()) {
			LogPrint("masternode", "dsee - Ignore Not Empty ScriptSig %s\n", vin.prevout.hash.ToString());
			Misbehaving(pfrom->GetId(), 100, 33);
			return;
		}
		...
		if (!obfuScationSigner.VerifyMessage(pubkey, vchSig, strMessage, errorMessage)) {
			...
			Misbehaving(pfrom->GetId(), 100, 34);
			return;
		}

		if (Params().NetworkID() == CBaseChainParams::MAIN) {
			if (addr.GetPort() != 5656) return;
		}
		else if (addr.GetPort() == 5656)
			return;
		...
		if (!obfuScationSigner.IsVinAssociatedWithPubkey(vin, pubkey)) {
			...
			Misbehaving(pfrom->GetId(), 100, 35);
			return;
		}
		...
		if (fAcceptable) {
			if (GetInputAge(vin) < MASTERNODE_MIN_CONFIRMATIONS) {
				...
				Misbehaving(pfrom->GetId(), 20, 36);
				return;
			}

			// verify that sig time is legit in past
			// should be at least not earlier than block when 1000 Proelio tx got MASTERNODE_MIN_CONFIRMATIONS
			uint256 hashBlock = 0;
			CTransaction tx2;
			GetTransaction(vin.prevout.hash, tx2, hashBlock, true);
			BlockMap::iterator mi = mapBlockIndex.find(hashBlock);
			if (mi != mapBlockIndex.end() && (*mi).second) {
				CBlockIndex* pMNIndex = (*mi).second;                                                        // block for 25000 PEO tx -> 1 confirmation
			...
			if (state.IsInvalid(nDoS)) {
				...
				if (nDoS > 0)
					Misbehaving(pfrom->GetId(), nDoS, 37);
			}
		}
	}
